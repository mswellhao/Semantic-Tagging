...
from fairseq.modules import AdaptiveSoftmax, FairseqDropout, GradMultiply
...
class LSTMModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        tgt_embed = Embedding(len(task.target_dictionary), args.decoder_embed_dim, task.target_dictionary.pad())

        encoder = LSTMTagEncoder(
            ...
            embed_tags=tgt_embed,
            max_source_positions=max_source_positions,
            bert_path=args.bert_path,
            freeze_bert=args.freeze_bert
        )
        decoder = LSTMTagDecoder(
    ...
    def forward(
        ...
        encoder_out = self.encoder(src_tokens, src_lengths=src_lengths, tag_tokens=tag_tokens)
...
class LSTMTagEncoder(FairseqEncoder):
    """LSTM encoder."""
    def __init__(
        self, dictionary, embed_dim=512, hidden_size=512, num_layers=1,
        dropout_in=0.1, dropout_out=0.1, bidirectional=False,
        left_pad=True, pretrained_embed=None, embed_tags=None, padding_idx=None,
    ...
    def load_pretrain_model(self):
        model_states = torch.load(self.bert_path)['model']
    ...
    def forward(
        ...
        """
        # embed tokens
        # print("src_tokens : "+str(src_tokens[:2]))
        # print("tag_tokens : "+str(tag_tokens[:2]))
        if self.left_pad:
            ...
            tag_tokens = utils.convert_padding_direction(
                tag_tokens,
                torch.zeros_like(tag_tokens).fill_(self.padding_idx),
                left_to_right=True,
            )
        ...
        if self.bert_path is not None:
            ...
            # x = GradMultiply.apply(x, 1 / 5) 
        else:
            x = self.embed_tokens(src_tokens)


        x = torch.cat([x, self.embed_tags(tag_tokens)], axis=2)
        ...
        packed_x = nn.utils.rnn.pack_padded_sequence(
            x, src_lengths.data.cpu(), enforce_sorted=enforce_sorted
        )
...
class LSTMTagDecoder(FairseqIncrementalDecoder):
    """LSTM decoder."""
    def __init__(
...
@register_model_architecture('lstm_tag', 'lstm_tag')
...
# @register_model_architecture('lstm_tag', 'lstm_tag_parsing')
# def lstm_parsing(args):
#     args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 200)
#     args.encoder_layers = getattr(args, 'encoder_layers', 1)
#     args.encoder_hidden_size = getattr(args, 'encoder_hidden_size', 100)
...
@register_model_architecture('lstm_tag', 'lstm_tag_parsing_large')
def lstm_parsing(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
    args.encoder_layers = getattr(args, 'encoder_layers', 1)
